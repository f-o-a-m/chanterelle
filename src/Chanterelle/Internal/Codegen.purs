...
generatePS = do
  ...
  let
    mextraJsonDir = case project.spec of
  ...
  liftAff $ case mextraJsonDir of
    ...
    Just extraJsonDir -> do
      ...
      errs <- PSWeb3Gen.generatePS
        ( psArgs
            { jsonDir = extraJsonDir
            , truffle = false
            }
        )
...
projectPSArgs (ChanterelleProject project) =
  let
    (ChanterelleProjectSpec spec) = project.spec
  in
    { jsonDir: project.jsonOut
...
loadAbi abiFile = do
  ...
  parseAbi json =
    let
      mabi = json ^? _Object <<< ix "abi"
    in
      note ("ABI field missing in " <> abiFile) mabi >>= lmap printJsonDecodeError <<< decodeJson
