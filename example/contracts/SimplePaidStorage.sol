...
contract SimplePaidStorage {
    ...
    function updateCount(uint _newCount) external {
        require(_token.balanceOf(msg.sender) >= 1, "Insufficient tokens to update count");
        require(_token.allowance(msg.sender, address(this)) >= 1, "Token allowance not set for contract");
    ...
    // In case the owner wants to withdraw any stuck ether
