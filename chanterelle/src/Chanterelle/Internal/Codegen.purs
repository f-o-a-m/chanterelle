...
generatePS p@(ChanterelleProject project) = do
  ...
  void <<< for project.modules $ \(ChanterelleModule mod) -> do
      abi <- liftAff $ loadAbi p mod.jsonPath
      let psModule = generatePSModule p abi mod.moduleName
      liftEff $ log Debug $ "generating purescript for " <> mod.moduleName
      liftAff $ assertDirectory (Path.dirname mod.pursPath)
      liftEff $ log Info $ "writing PureScript bindings for " <> mod.moduleName
      liftAff $ FS.writeTextFile UTF8 mod.pursPath psModule

projectPSArgs :: ChanterelleProject -> PSWeb3Gen.GeneratorOptions
projectPSArgs (ChanterelleProject project) = let (ChanterelleProjectSpec spec) = project.spec
...
generatePSModule :: ChanterelleProject
                 -> PSWeb3Gen.Abi
                 -> String
                 -> String
...
loadAbi (ChanterelleProject project) abiFile = do
  ...
  either (throwError <<< error) pure $ parseAbi json

  where parseAbi json = let mabi = json ^? _Object <<< ix "abi"
